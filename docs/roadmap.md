...
| 31 | **Identification supervised metric learning unblocked** | ece3c94 | `_sample_positive_index` previously returned `anchor_index`, so triplets only taught augmentation invariance. Now every sample carries a normalized `(card_name, set_name)` key (lowercase + strip + collapse-whitespace; deliberately conservative — no punctuation stripping that could merge "Mewtwo" with "Mewtwo & Mew"). `key_index` lookup; singletons (no different-cert positive) are dropped. Triplet loss sees genuine printing-variant positives — supervised metric learning enabled with **no new data**. 100 tests passing. |
| 32 | **Catalog DB-side ingest** | d609c63 | `ml/scripts/sync_catalog_db.py` — pass 1 upserts unique `(game, set_code)` into `card_sets`; pass 2 streams JSONL, looks up embedding from `reference_embeddings.npz`, computes `canonical_phash` from the front image (not previously precomputed), upserts into `card_variants` keyed `(game, set_id, card_number)`. Async SQLAlchemy + `INSERT ... ON CONFLICT DO UPDATE` for idempotency. Slot 4 of `daily_cycle.sh` (skips with rc=0 when `DATABASE_URL` is unset so dev boxes don't fail the cycle). 10 unit tests on the prep layer. Closes the gap where the manufacturer reference flywheel and the production identification pgvector path ran on different stores. |
| 33 | **Cert-page 500 fix — Phase-1 partial grades render** | bf70ca9 | `GradeOut.{centering, corners, edges, final}` were non-nullable in the public Pydantic schema, but `compute_psa_final` returns None whenever corners or surface is missing — which is *every* Phase-1 cert with skeleton trainers. `model_validate(g)` was 500'ing on every cert request. Relaxed to `float \| None`, mirrored in `apps/web/lib/cert.ts`, added amber "Preliminary subgrades — final grade unavailable" banner when `primary.final === null`. Regression test added. |
| 34 | **Submit endpoint broker-failure handling** | b123a6c | `apps/api/grader/routers/submissions.py` — on `process_submission.delay` exception, revert `sub.status = SubmissionStatus.CAPTURING` and raise HTTPException(503, `{reason: broker_unavailable, retry_after: 30}`). Stops submissions silently stranding when Celery is down. The early-return guard at line 220 already protects against double-enqueue. Reconciler beat intentionally NOT built (different problem). |
| 35 | **apps/api → uv editable dep on grader-ml** | 287f04c | Replaced 15 `_ML_ROOT = parents[4]/"ml"` sys.path-bootstrap blocks with a real editable install via `[tool.uv.sources] grader-ml = { path = "../../ml", editable = true }`. Import paths unchanged (hatch wheel exposes `pipelines`, `data`, etc. at top level). 113/24 unit-pure tests green; pre-existing pgvector/asyncpg/no-postgres failures are unaffected. Deletes a depth-counting footgun across services + workers + tests. |
| 36 | **Capture wizard — flash + tilt shots** | d66834c | Added `front_full_flash` + `tilt_30` as `required: false` rows after the corners in `WIZARD_SHOTS`. Backend was already ready (ShotKind enum + quality thresholds in `THRESHOLDS_BY_SHOT`). Archives the data for the future substrate (#6) + holographic (#3) detectors that consume them. UI-only; pure +12 lines. |
| 37 | **Web frontend → real Clerk JWTs** | c37cf03 | `@clerk/nextjs@7.3.0` mounted via `<ClerkProvider>` in `app/layout.tsx`. Middleware gates `/grade/:path*` (auth required); `/cert/:path*` stays public. New `useAuthedFetch()` hook in `lib/submission.ts` emits `Authorization: Bearer <jwt>` from `useAuth().getToken()`; falls back to `Authorization: Dev <NEXT_PUBLIC_DEV_CLERK_ID>` when Clerk env is unset so tests/local dev keep working without keys. API-side `dev_auth_enabled` flag intentionally not flipped — toggle in production env. |
| 39 | **Holographic-parallax counterfeit detector (#3 of 7)** | a169349 | `ml/pipelines/counterfeit/holographic/`. Dense Farnebäck optical flow between dewarped front + tilt_30 canonicals, restricted to a foil mask (CIELAB chroma > 30 AND HSV saturation > 80; opened with a 3×3 ellipse). Score = logistic-squashed flow-ratio inside-vs-outside the mask. Anchors: ≥3× ratio (real holo) → ~0.95; ~1× ratio (flat fake) → ~0.05. Graceful UNVERIFIED on tilt_not_captured / no_holo_region / shape_mismatch / flow_computation_failed. Pipeline_runner adds TILT_30 to OPTIONAL_SHOTS so dewarp soft-fails. 13 + 7 + 1 = 21 new tests. **Counterfeit ensemble now 5/7 wired.** Documented limits: false-negatives on silver/gold mirror foil; false-positives on highly-saturated full-art non-foil cards. |
| 38 | **Typography counterfeit detector (#5 of 7)** | 4194511 | `ml/pipelines/counterfeit/typography/`. RapidOCR (`rapidocr-onnxruntime`, PP-OCR weights via ONNX Runtime, no Paddle dep) on the top-10% title ROI of the canonical 750×1050 dewarped front. Case-insensitive normalized Levenshtein distance vs. the identified card name. Logistic squash midpoint 0.35, slope 12: exact match → 0.985, 1-char typo → ~0.96, totally wrong → ~0.02. Graceful UNVERIFIED on no-identified-name / invalid-image / OCR-import-missing — never raises into the ensemble. Wired through `ensemble.py` (TYPOGRAPHY_AUTHENTIC=0.65, COUNTERFEIT=0.35, MIN_CONFIDENCE=0.4 — placeholder thresholds, recalibration tool ratchets later), recalibration coverage (`CURRENT_THRESHOLDS` + `_PATCH_CONSTANT_NAMES`), pipeline_runner stage 3.5. 18 detector tests + service + pipeline tests; 112 ml tests passing. Counterfeit ensemble now **4/7 wired**. |
...
| 8 | ~~**Typography counterfeit detector**~~ | ML | done 2026-05-04 (4194511) | Detector #5 wired via RapidOCR + Levenshtein. Next iteration: per-glyph 2D-correlation against canonical font templates rendered from manufacturer references — catches counterfeits that OCR perfectly but use a substitute font with subtly-wrong stroke contrast / x-height / serif geometry. |
...
| 10 | ~~**Holographic parallax detector**~~ | ML | done 2026-05-05 (a169349) | Detector #3 wired via Farnebäck flow + foil mask. Next iteration: multi-angle tilt (3+ angles) so flow-variance grows linearly with angle count, sharpening separation from flat counterfeit foil. |
